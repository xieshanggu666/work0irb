 ...
 *    全部序列化；占用表读档后由列车位置重建（跨格中途的半格状态不恢复，列车吸附回
 *    已落定格重新寻路），兼容无铁路字段的旧存档。
 */
FG.Railway = class Railway {
  ...
  removeTrain(tr) {
    ...
    // 按 id 清掉该车的全部占用条目，否则新格会留下幽灵占用，永久挡住后续列车
    for (const [k, id] of this.occupy) {
      if (id === tr.id) this.occupy.delete(k);
    }
    tr._dead = true;
  }
  ...
  deserialize(data) {
    ...
    for (const st of (data.trains || [])) {
      ...
      // 统一吸附回已落定格（moveTimer 清零、path 作废重新寻路），否则残留的跨格计时
      // 会让列车在没有占用权的情况下凭空走入新格，与后车重叠
      tr.moveTimer = 0;
      tr.path = null;
      this.trains.push(tr);
  ...
  freeNeighbor(x, y) {
    for (let d = 0; d < 4; d++) {
      ...
      if (this.nodes.has(nk) && !this.occupy.has(nk)) return { x: nx, y: ny };
    }
    return null;
  }
};
