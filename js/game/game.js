...
FG.Game = class Game {
  ...
  removeTrainSelection() {
    ...
    // 跨格中途车体格与占用格可能不一致：货物落到占用权实际所在格
    let gx = tr.x, gy = tr.y;
    for (const [k, id] of this.railway.occupy) {
      if (id === tr.id) { [gx, gy] = k.split(',').map(Number); break; }
    }
    const cargoN = tr.cargoTotal();
    for (const s of tr.cargo) this.map.pileAdd(gx, gy, s.type, s.count);
    this.railway.removeTrain(tr);
    this.selection = null;
    this.logMsg('已解编列车 ' + tr.id + '：车上 ' + cargoN + ' 件货物已落到地面堆', 'info');
